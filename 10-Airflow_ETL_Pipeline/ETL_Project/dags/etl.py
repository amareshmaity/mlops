...
with DAG(
    ...
    ## Step 1: Create table if it doesnot exist
    @task
    def create_table():
        ...
        ## We will run the postgre inside docker as container
        postgres_hook.run(create_table_query)

    ## Step 2: Extract the NASA API data (APOD-Astronopy Picture of the Day) - Extract pipeline
    ...
    def transform_apod_data(response):
        apod_data={
            'title': response.get('title', ''), # if no data then keep it ''
